// <copyright file="GcHeapGrowthRateHistogramCollector.cs" company="NetMetric">
...
/// <summary>
/// A collector that calculates the heap growth rate (Δheap/s) by reading heap size from EventCounters.
/// It writes the growth values (Δheap/s > 0) to a real bucket histogram with a 60-second Tumbling window.
/// </summary>
public sealed class GcHeapGrowthRateHistogramCollector : IMetricCollector
{
    ...
    // Bounds for heap growth rate in bytes/s: 32KB/s .. 512MB/s
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="GcHeapGrowthRateHistogramCollector"/> class.
    /// </summary>
    ...
    public GcHeapGrowthRateHistogramCollector(IMetricFactory factory, IRuntimeGcMetricsSource src)
    {
    ...
    /// <summary>
    /// Collects the heap growth rate asynchronously and records it in a bucket histogram.
    /// </summary>
    ...
    /// <remarks>
    /// This method calculates the heap growth rate by comparing the current heap size to the previous one
    /// and observes the rate in the histogram. Only positive growth rates (Δheap/s > 0) are recorded.
    ...
    public Task<IMetric?> CollectAsync(CancellationToken ct = default)
    {
        ...
        // Create histogram with a 60s tumbling window for heap growth rate
        var hist = _factory.Histogram("runtime.gc.heap.growth.rate.hist", "Heap Growth Rate Histogram (bytes/s)").WithBounds(_bounds).WithWindow(MetricWindowPolicy.Tumbling(TimeSpan.FromSeconds(60))).Build();
        ...
        var rate = deltaBytes / elapsedSec; // Positive for growth

        if (rate > 0 && double.IsFinite(rate))
        {
            hist.Observe(rate); // Observe in histogram if rate is positive
        }

        return Task.FromResult<IMetric?>(hist); // Return the histogram with observed growth rate
    }
